...
All tests are xfail until PR 3 creates those scripts.
"""
...
@pytest.mark.xfail(reason="05_load_iluc_matrices.py not yet created (PR 3)")
...
@pytest.mark.xfail(reason="06_interpolate_matrices.py not yet created (PR 3)")
def test_interpolated_json_contains_all_years(tmp_path, iluc_matrix_csv_path):
    ...
    out_dir.mkdir()
    interp.run(anchor_csv_map={2008: iluc_matrix_csv_path, 2017: iluc_matrix_csv_path, 2024: iluc_matrix_csv_path},
               out_dir=out_dir)
...
def test_interpolated_json_schema_valid(tmp_path, iluc_matrix_csv_path):
    ...
    assert "matrices"     in data
    # Each matrix year should have from_class → to_class → float
